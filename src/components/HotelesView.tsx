...
import { sedesData } from '@/app/hoteles/sedesData';
import { Navigation } from '@/components/Navigation';

type Hotel = (typeof hotelesData)[number];
type Sede = (typeof sedesData)[number];
...
function GoogleMarkersMap({
  ...
  useEffect(() => {
    ...
    const renderMap = async () => {
      ...
      type MapItem = (Hotel | Sede) & { isSede?: boolean };
      const allTargets: MapItem[] = [...targets] as MapItem[];
      if (mapMode === 'all') {
        allTargets.push(
          ...sedesData.map((s) => ({ ...s, isSede: true } as MapItem))
        );
      }

      for (const item of allTargets) {
        const isSedeItem = item.isSede === true;
        const cacheKey = item.mapQuery ?? item.name;
        ...
        const infoWindow = new google.maps.InfoWindow({
          content: `<div style="font-family: Arial, sans-serif; font-size: 13px;"><strong>${item.name}</strong><br/>${(item as Hotel).zona || (item as Sede).direccion}</div>`,
        });
      ...
      <div className="flex-1 flex flex-col md:flex-row w-full md:h-[calc(100vh-80px)]">
        ...
        <div className="w-full md:w-1/2 lg:w-[45%] flex flex-col border-r border-border md:h-full bg-card/50 order-last md:order-first">
          <div className="p-4 md:p-6 border-b border-border bg-background z-10 shrink-0 space-y-4 shadow-sm relative">
            ...
            <div className="flex bg-muted/50 p-1 rounded-lg border border-border mb-4">
              <button
                ...
                className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${viewMode === 'hoteles' ? 'bg-background shadow text-[#0b697d]' : 'text-muted-foreground hover:text-foreground'}`}
              >
                Hoteles
              </button>
              <button
                onClick={() => setViewMode('sedes')}
                className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${viewMode === 'sedes' ? 'bg-background shadow text-[#0b697d]' : 'text-muted-foreground hover:text-foreground'}`}
              >
                Sedes
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/10">
            {viewMode === 'sedes' && sedesData.map((sede) => (
              <motion.div
                key={'sede-' + sede.id}
                whileHover={{ y: -2 }}
                onClick={() => {
              ...
              >
                <div className="p-4 md:p-5 flex flex-col justify-between">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h3 className="text-lg font-bold text-[#0b697d] dark:text-[#2eb4cc] leading-tight">
                        {sede.name}
                      </h3>
                  ...
                  <div className="mt-4 p-3 bg-muted/50 rounded-lg dark:bg-muted/20">
                    <p className="text-sm text-foreground space-x-1">
                      <strong className="text-[#0b697d] dark:text-[#2eb4cc]">Actividades:</strong>
                      <span>{sede.notas}</span>
                    </p>
                  </div>
                </div>
              </motion.div>
            ...
            {viewMode === 'hoteles' && filteredHotels.map((hotel) => (
              <motion.div
                key={hotel.id}
                ...
                <div className="w-28 sm:w-36 aspect-square shrink-0 bg-linear-to-br from-muted/50 to-muted relative flex items-center justify-center border-r border-border/50 overflow-hidden">
                   {hotel.imagenUrl ? (
                     ...
                     <div className="absolute inset-0 flex flex-col items-center justify-center p-2 text-center text-muted-foreground/50">
                       <span className="text-2xl sm:text-3xl opacity-60">ðŸ¨</span>
            ...
            {viewMode === 'hoteles' && filteredHotels.length === 0 && (
